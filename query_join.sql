...
-- 6. Selezionare tutti i docenti che insegnano nel Dipartimento di Matematica (54)--ERROR 
SELECT CONCAT(`teachers`.`surname`, " " , `teachers`.`name`) AS "Teachers of Mathematics Department"
FROM `teachers`
INNER JOIN `course_teacher`
	ON `course_teacher`.`teacher_id`=`teachers`.`id`
INNER JOIN `courses`
